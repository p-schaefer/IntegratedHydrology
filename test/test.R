...
final_attributes_sub<-fasttrib_points(
  ...
  temp_dir=NULL,
  verbose=T,
  backend="data.table", #c("SQLite","tibble","data.table")
  SQLdb_path=":memory:"
)

# kp<-sapply(as_tibble(df),function(x) !all(is.na(x)))
# kp<-names(kp)[kp]
#
# df3<-df2
# df<-as_tibble(df)
...
#   dplyr::mutate(
#     dplyr::across(
#       tidyselect::any_of(loi_cols3), # tidyselect::any_of
#       ~.*(!!rlang::sym("iFLS")),
#       .names=paste0("{.col}_","iFLS")
#     )
#   )
#
# # library(data.table)
...
#
#                       dplyr::mutate(x,
#                                     dplyr::across(tidyselect::any_of(c1),
#                                                   ~.*(!!rlang::sym("carb")),
#                                                   .names=paste0("{.col}_","carb")
#                                     )
#                       )
#                     }))
#
