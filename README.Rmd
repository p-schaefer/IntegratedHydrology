...
toy_dem[!is.na(burn_lyr)]<-toy_dem[!is.na(burn_lyr)]-5 # rudimentary stream burning
...
# temporary files won't accumulate eating valuable hardrive space.
...
# Run hydrology processing tools 
...
hydro_out2<-try(generate_pwisedist(
  input=hydro_out,
  return_products=T,
  temp_dir=NULL,
  verbose=F
...
mapview(hydro_out$subbasins,zcol="link_id",legend=F,layer.name=".",alpha.regions=0.1)+
  ...
  mapview(point_catchments,legend=F, color = "black", col.regions = "white", lwd = 5,alpha.regions=0.8,layer.name=".....")+
  mapview(subbasin_catchments,legend=F, color = "Yellow", col.regions = "white", lwd = 5,alpha.regions=0.8,layer.name="......")
...
# Instead of processing loi separately, they can be added to the completed hydrology workflow, 
# and added to the existing .zip file for convenient file storage/organization. If
# run separately (as above), "num_rast.tif" and/or "cat_rast.tif" will need to manually
# be added to the .zip file before points can be attributed.
...
### 3.6 Calculate (weighted) spatial summaries at sampleing points `attrib_points()`
...
### 3.7 Calculate (weighted) spatial summaries at each reach `attrib_points()`
