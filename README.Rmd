...
Begin by bringing in the digital elevation model and using it to generate terrain products. 
...
The DEM must be processed in a way to remove depressions. Whitebox offers methods for breaching and filling DEMs to remove depressions. 

Another factor to consider at this step is whether to burn stream vectors into the DEM; ***ihydro*** impliments a simplified stream burning method that lowers the levation along the DEM by `burn_depth` meters. See  [here](https://proceedings.esri.com/library/userconf/proc99/proceed/papers/pap802/p802.htm#Trois) for more detailed approaches.
...
### 6.1 At specific sampling points with site specific attributes `spatweight_attr()`

The `spatweight_attr()` function can be run with a specification table to specify at which sites loi summaries should be calculated for, which loi to summarize, and type of numeric summary to use for those particular loi. 

```{r, fig.width = 7, fig.height = 7, message = F, error = F, warning = F}

final_attributes_sub<-spatweight_attr(
  input=hydro_out,
...
### 6.2 At all sampling points `spatweight_attr()`

```{r, fig.width = 7, fig.height = 7, message = FALSE, error = FALSE, warning = FALSE}

final_attributes<-spatweight_attr(
  input=hydro_out,
  loi_file=output_filename_loi,
...
# loi_names<-lapply(loi,names) %>% unlist()
# names(loi_names)<-loi_names
# loi_names<-map(loi_names,~c("mean","max","sd","distwtd_mean","distwtd_sd"))
# 
...
#   OS_combine=F,
#   target_streamseg=T, # This will set the target_o parameter as the stream segment
#   inv_function = function(x) {
#     (x * 0.001 + 1)^-1
#   },
#   remove_region=NULL,
...
### 6.3 Across entire study area `spatweight_attr()`
...
final_attributes_all<-spatweight_attr(
  input=hydro_out,
  loi_file=output_filename_loi,
  sample_points=NULL,
  link_id=NULL,
...
#   link_id=as.character(read_sf(file.path("/vsizip",hydro_out_sparse$outfile,"stream_links.shp"))$link_id),
#   loi=list(loi_names)
# )
# 
# spatweight_attr(
...
#   sample_points=hydro_out$snapped_points$site_id,
#   link_id=NULL,
#   weighting_scheme =  c("lumped", "iEucS",  "iFLS", "HAiFLS","iFLO",  "HAiFLO"),
...
* One big area of improvement will be the handling of the output .zip file structure.
