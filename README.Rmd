...
toy_dem[!is.na(burn_lyr)]<-toy_dem[!is.na(burn_lyr)]-5
...
# wbt_fill_burn(
#   dem=file.path(save_dir, "toy_dem_breached.tif"),
#   streams=file.path(save_dir, "toy_streams.shp"),
#   output=file.path(save_dir, "toy_dem_burned.tif")
# )
# 
# toy_dem<-rast(file.path(save_dir, "toy_dem_burned.tif"))
...
landuse_r_path <- system.file("extdata", "nc", "landuse_r.tif", package = "openSTARS")
...
read_sf(pointsources_path) %>% 
  mutate(pointsource=1) %>% 
  group_by(pointsource) %>% 
  summarize(across(everything(),head,1)) %>% 
  st_buffer(30) %>% 
  write_sf(file.path(save_dir, "pointsources.shp"))
...
if (F) {
  ...
  landuse <- system.file("extdata", "nc", "landuse_r.tif", package = "openSTARS")
...
mapview(hydro_out$subbasins,zcol="link_id",legend=F)+
  ...
  mapview(hydro_out$snapped_points,zcol="link_id",legend=F,color="yellow")+
  mapview(read_sf(file.path(save_dir, "sites.shp")),color="red")
...
point_catchments<-get_catchment( # retrieve sampling point catchment
  input=hydro_out,
  site_id_col="site_id",
  target_points=c("1","25","80")
)

mapview(hydro_out$subbasins,zcol="link_id",legend=F,layer.name=".")+
  mapview(hydro_out$stream_lines,zcol="link_id",legend=F,layer.name="..")+
  mapview(hydro_out$links,zcol="link_id",legend=F,layer.name="...")+
  mapview(hydro_out$snapped_points,zcol="site_id",legend=F,color="red",layer.name="....")+
  mapview(point_catchments,legend=F, color = "black", col.regions = "white", lwd = 5,alpha=0.8,layer.name=".....")+
  mapview(subbasin_catchments,legend=F, color = "Yellow", col.regions = "white", lwd = 5,alpha=0.8,layer.name="......")
...
hydro_out_comp2<-generate_pwisedist(
