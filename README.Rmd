...
cv_strats<-list(
  ...
  spatial=spatial_clustering_cv(comb_data, # using spatial information to leave out groups of nearby stations
                                cluster_function = "hclust", # we saw hclust did a decent job clustering sites previously
...
Next we build the final model. We will use the hyper-parameters that yielded the highest prediction accuracy using standard cross-validation.
...
prediction_data<-lkp %>% 
  ...
  filter(!if_any(contains("HAiFLO"),is.na))

```

And finally, we can predict across the landscape. We'll use the range of quantile predictions to estimate uncertainty in the model predictions.
...
Streams<-read_sf(file.path("/vsizip",hydro_out_sparse$outfile,"stream_lines.shp")) %>% 
  mutate(link_id=floor(link_id)) %>% #this will assign the predicted value to the entire reach, not just the unsampled portion
